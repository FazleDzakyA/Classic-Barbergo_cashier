  ...
  return (
    <div className="booking-page-container">
      ...
      <div className="booking-content-wrapper">
          ...
          >
            ...
            {!myBookings || myBookings.length === 0 ? (
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: '1.25rem' }}>
                {myBookings.map((bk) => {
                  const bObj = barbers?.find(b => Number(b.id) === Number(bk.barberId));
                  const bName = bObj?.name || 'Barber';

                  // Robust serviceIds parser (supports comma-separated string, array of strings, array of numbers)
                  ...
                  const sNames = parsedSids
                    .map(sid => allServices?.find(s => Number(s.id) === Number(sid))?.name)
                    .filter(Boolean)
                    .join(', ');
        ...
        <footer style={{ position: 'relative', zIndex: 5, marginTop: '3.5rem', background: 'rgba(18, 18, 23, 0.95)', border: '1px solid rgba(212, 175, 55, 0.35)', borderRadius: '24px', padding: '2.5rem 2rem', boxShadow: '0 20px 50px rgba(0,0,0,0.8), inset 0 1px 0 rgba(255,255,255,0.1)', backdropFilter: 'blur(20px)' }}>
          <div style={{ display: 'flex', flexDirection: 'column', gap: '3rem' }}>
            
            {/* SECTION 1: MAPS & LOKASI DETAIL */}
            <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fit, minmax(300px, 1fr))', gap: '2rem', alignItems: 'stretch' }}>
              {/* Left: Info Barbershop & Contacts */}
              <div style={{ background: 'rgba(24, 24, 30, 0.8)', border: '1px solid rgba(212, 175, 55, 0.25)', borderRadius: '20px', padding: '1.75rem', display: 'flex', flexDirection: 'column', justifyContent: 'space-between' }}>
                <div>
                ...
                <div style={{ marginTop: '1.75rem', paddingTop: '1.25rem', borderTop: '1px solid rgba(255,255,255,0.08)' }}>
                  <a 
                    ...
                    rel="noreferrer"
                    className="btn"
                    style={{ width: '100%', background: '#D4AF37', color: '#000', fontWeight: 900, borderRadius: '12px', padding: '0.75rem 1rem', fontSize: '0.88rem', display: 'inline-flex', alignItems: 'center', justifyContent: 'center', gap: '0.4rem', textDecoration: 'none' }}
                  >
                    <MapPin size={18} />
